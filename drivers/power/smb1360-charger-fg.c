...
#define BAT_ID_SEC_UV						500000
#define BAT_ID_THIR_UV						780000
#define BAT_ID_THRESHOLD_UV				900000
#define BAT_ID_FOU_UV						1000000
#define BAT_ID_FIV_UV					1220000
#define BAT_ID_HIGH_UV						1800000
#define BAT_ID1                                             "HB3080G1EBC"
#define BAT_ID2                                             "HB3080G1EBW"
#define BAT_ID3												"HB405979ECW_DESAY_ATL"
#define BAT_ID4												"HB405979ECW_SCUD_COSLIGHT"
#define BAT_ID5												"HB405979ECW_SUNWODA_SONY"
#define BAT_ID6												"HB405979ECW_SCUD_LG"

#define FASTCHARGING_CURRENT_LIMIT     1000
#define NONSTANDARD_FASTCHARGING_CURRENT_LIMIT 1000
...
struct smb1360_chip {
	...
	int				cool_higher_bat_decidegc;
	int				warm_bat_decidegc;
	int				cool_bat_mv;
	int				warm_bat_mv;
	int				cool_bat_ma;
	int				cool_higher_bat_ma;
	int				warm_bat_ma;
...
static int charge_compensation_rank_current[] = {
	100, 150, 200, 450, 550, 600, 750, 900, 1050, 1200, 1350, 1500,
};
static int charge_compensation_current[] = {
...
static int smb1360_set_jeita_comp_current(struct smb1360_chip *chip,
							int current_ma);
...
static int smb1360_set_appropriate_usb_current(struct smb1360_chip *chip)
{
	...
	current_ma = min(current_ma,FASTCHARGING_CURRENT_LIMIT);
        if(current_ma > CURRENT_500_MA){
			...
			if(chip->project_id == HUAWEI_SLA)
				current_ma = min(current_ma, path_current);
	}
	if(chip->project_id == HUAWEI_SLA) {
		if (chip->batt_warm)
			smb1360_set_jeita_comp_current(chip, chip->warm_bat_ma);
		else if (chip->batt_cool)
			smb1360_set_jeita_comp_current(chip, chip->cool_bat_ma);
	} else {
		if (chip->batt_warm)
	...
	charging_current_now_set = current_ma;
	...
	if (current_ma <= CURRENT_100_MA) {
		...
		if (chip->rsense_10mohm)
			current_ma /= 2;
		if(chip->project_id == HUAWEI_SLA && chip->batt_cool_higher)
			current_ma = min(current_ma, chip->cool_higher_bat_ma);
		...
		/* AC input 1A , config fastcharge 1.05A insted of 1A*/
		if(current_ma == FASTCHARGING_CURRENT_LIMIT && chip->project_id == HUAWEI_SLA)
                       i++;
...
static void smb1360_monitor_charging_work(struct work_struct *work)
{
	int batt_temperature = 0;
	bool usb_present = 0;
	struct smb1360_chip *chip = container_of(work,
			struct smb1360_chip, monitor_charging_work.work);
	usb_present = chip->usb_present;
	if (usb_present) {
		batt_temperature = smb1360_get_prop_batt_temp(chip);
		/*
		 ...
		 */
		if ((batt_temperature > chip->cool_bat_decidegc)
			&& (batt_temperature <= chip->cool_higher_bat_decidegc)) {
			chip->batt_cool_higher = true;
			smb1360_set_appropriate_usb_current(chip);
		} else {
			if (batt_temperature > chip->cool_higher_bat_decidegc && chip->batt_cool_higher) {
				chip->batt_cool_higher = false;
				smb1360_set_appropriate_usb_current(chip);
			}
		}

		schedule_delayed_work(&chip->monitor_charging_work,
				msecs_to_jiffies(HIGH_SOC_REPORT_TIME));
	}
}
							...
							int current_ma)
{
	...
	for (i = ARRAY_SIZE(charge_compensation_rank_current) - 1; i >= 0; i--) {
		if (charge_compensation_rank_current[i] <= current_ma) {
			current_ma = charge_compensation_rank_current[i];
			break;
		}
	}
	if (i < 0) {
		pr_debug("Couldn't find charge_compensation_rank_current %dmA\n", current_ma);
		i = 0;
	}
	for (i = ARRAY_SIZE(charge_compensation_current) - 1; i >= 0; i--) {
		if (charge_compensation_current[i] == current_ma)
			break;
	}
	if (i < 0) {
		pr_debug("Couldn't find charge_compensation_current %dmA\n", current_ma);
		i = 0;
	}
...
static void smb1360_set_prop_in_thermal(struct smb1360_chip *chip, int val);
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_IN_THERMAL:
		smb1360_set_prop_in_thermal(chip,val->intval);
		break;
...
static void smb1360_set_prop_in_thermal(struct smb1360_chip *chip, int val)
{
	int i = 0, rc = 0, current_limit = 0;
	if (val) {
		hot_design_current = val;
		if(chip->iin_thermal_enable)
		{
			current_limit = min(val,charging_current_now_set);
			for (i = ARRAY_SIZE(input_current_limit) - 1; i >= 0; i--) {
...
static int smb1360_otp_set_rslow(struct smb1360_chip *chip)
{
	...
	for (i = 0; i < ARRAY_SIZE(reg_val_mapping_rid); i++)
	{
		/*for HB405979ECW_ATL and HB405979ECW_SUNWODA*/
		if((chip->connected_rid >= BAT_ID_LOW_UV && chip->connected_rid < BAT_ID_THIR_UV) || (chip->connected_rid >= BAT_ID_FOU_UV && chip->connected_rid < BAT_ID_FIV_UV))
		{
			address = reg_val_mapping_rid[i][0];
			data = reg_val_mapping_rid[i][1];
		}
		else
		{
			pr_err("Failed to rslow for RID=%d\n", chip->connected_rid);
			return -EINVAL;
		}
		pr_debug("Writing reg_add=%x value=%x\n", address, data);
...
static int smb1360_otg_rslow_cfg(struct smb1360_chip *chip)
{
	int rc = 0;
	rc = smb1360_enable_fg_access(chip);
	if (rc)
...
static int smb1360_delayed_hw_init(struct smb1360_chip *chip)
{
	...
	if (chip->otp_rslow_config) {
		/*the qcom patch for the battery of HB405979ECW_SUNWODA_SONY and HB405979ECW_DESAY_ATL*/
		if (chip->project_id == HUAWEI_SLA) {
			rc = smb1360_otg_rslow_cfg(chip);
			if (rc) {
				pr_err("Unable to config rslow cfg rc = %d\n", rc);
				return rc;
			}
		}
	}
...
static void smb_add_battype_app_info(u32 batt_id_uv,int project_id)
{
	if(project_id == HUAWEI_SLA){
		if (batt_id_uv > BAT_ID_LOW_UV && batt_id_uv < BAT_ID_THIR_UV) {
			if (!app_info_set("battery_id", BAT_ID3)) {
				pr_err("set battery_id ver to app info successfully!\n");
			} else {
				pr_debug("set battery_id %s failed!\n", BAT_ID3);
			}
		} else if (batt_id_uv >= BAT_ID_THIR_UV && batt_id_uv < BAT_ID_FOU_UV) {
			if (!app_info_set("battery_id", BAT_ID4)) {
				pr_err("set battery_id ver to app info successfully!\n");
			} else {
				pr_err("set battery_id %s failed!\n", BAT_ID4);
			}
		} else if (batt_id_uv >= BAT_ID_FOU_UV && batt_id_uv < BAT_ID_FIV_UV) {
			if (!app_info_set("battery_id", BAT_ID5)) {
				pr_err("set battery_id ver to app info successfully!\n");
			} else {
				pr_err("set battery_id %s failed!\n", BAT_ID5);
			}
		} else if (batt_id_uv >= BAT_ID_FIV_UV && batt_id_uv < BAT_ID_HIGH_UV) {
...
static int smb1360_parse_jeita_params(struct smb1360_chip *chip)
{
	...
	if (of_property_read_bool(node, "qcom,soft-jeita-supported")) {
		...
		rc = of_property_read_u32(node, "qcom,cool-higher-bat-decidegc",
						&chip->cool_higher_bat_decidegc);
		if (rc) {
			pr_err("cool_higher_bat_decidegc property error, rc = %d\n",
								rc);
		}
		rc = of_property_read_u32(node, "qcom,cool-higher-bat-ma",
						&chip->cool_higher_bat_ma);
		if (rc) {
			pr_err("cool_higher_bat_ma property error, rc = %d\n", rc);
		}
...
static int smb_parse_dt(struct smb1360_chip *chip)
{
	...
	chip->iin_thermal_enable = of_property_read_bool(node,
					"qcom,iin-thermal-enable");
	...
	chip->otp_rslow_config = of_property_read_bool(node,"qcom,otp-rslow-cfg");
	return 0;
}
static struct smb1360_chip *global_smb1360_chip;

static char DUMP_CHG[] = {0x00, 0x05, 0x06, 0x7, 0x0A, 0x0D, 0x13, 0x14, 0x41, 0x42, 0x48, 0x4A, 0x4B};
