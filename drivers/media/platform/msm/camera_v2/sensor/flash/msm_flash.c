...
void msm_gpio_light_brightness_set(struct led_classdev *led_cdev,
				enum led_brightness value)
{
	int32_t rc = 0;
	CDBG("%s:Enter, value:%d\n", __func__, value);
	g_flash_ctrl->func_tbl = &flash_table[1]->func_tbl;

	if (value == 0) {
		rc = g_flash_ctrl->func_tbl->camera_flash_release(g_flash_ctrl);
		if (rc < 0) {
			pr_err("%s:flash release fail\n", __func__);
			return;
		}
	} else {
		rc = g_flash_ctrl->func_tbl->camera_flash_init(g_flash_ctrl, NULL);
		if (rc < 0) {
			pr_err("%s:flash init fail\n", __func__);
			return;
		}

		rc = g_flash_ctrl->func_tbl->camera_flash_low(g_flash_ctrl, NULL);
		if (rc < 0) {
			pr_err("%s:flash low fail\n", __func__);
			return;
		}
		g_flash_ctrl->flash_state = MSM_CAMERA_FLASH_TEST_MODE;
	}

	return;
}

static struct led_classdev msm_gpio_light = {
	.name                   = "torch-light0",
	.brightness_set = msm_gpio_light_brightness_set,
	.brightness             = LED_OFF,
};

static int32_t msm_gpio_light_create_classdev(struct platform_device *pdev,
				void *data)
{
	int32_t rc = 0;
	struct msm_flash_ctrl_t *fctrl =
		(struct msm_flash_ctrl_t *)data;
	struct msm_camera_power_ctrl_t *flashgpio_ctrl;

	CDBG("gpio light create classdev\n");
	if (!fctrl) {
		pr_err("Invalid fctrl\n");
		return -EINVAL;
	}

	flashgpio_ctrl = &(fctrl->power_info);
	if (flashgpio_ctrl != NULL) {
		rc = led_classdev_register(&pdev->dev,
				&msm_gpio_light);
		if (rc) {
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	flashgpio_ctrl = &(flash_ctrl->power_info);
	rc = msm_camera_pinctrl_init(&(flashgpio_ctrl->pinctrl_info), flashgpio_ctrl->dev);
	if (rc < 0) {
		pr_err("%s:%d Initialization of pinctrl failed\n",
				__func__, __LINE__);
		flashgpio_ctrl->cam_pinctrl_status = 0;
	} else {
		flashgpio_ctrl->cam_pinctrl_status = 1;
	}

	if (flashgpio_ctrl->gpio_conf->cam_gpio_req_tbl_size != 0) {
		rc = msm_camera_request_gpio_table(
			flashgpio_ctrl->gpio_conf->cam_gpio_req_tbl,
			flashgpio_ctrl->gpio_conf->cam_gpio_req_tbl_size, 1);
	}

	if (flashgpio_ctrl->cam_pinctrl_status) {
		rc = pinctrl_select_state(flashgpio_ctrl->pinctrl_info.pinctrl,
			flashgpio_ctrl->pinctrl_info.gpio_state_active);
		if (rc)
...
{
	int32_t rc = 0;
	struct msm_camera_gpio_conf *flashgpio_conf;

	flashgpio_conf = flash_ctrl->power_info.gpio_conf;

	CDBG("%s:%d gpio set flash_en val %d flash_now %d\n",
				__func__, __LINE__,
				flashgpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_FL_EN],
				flashgpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_FL_NOW]);
	/* turn on torch led sequence flash_en 0 flash_now 1*/
	if ((flashgpio_conf->gpio_num_info->valid[SENSOR_GPIO_FL_EN] == 1)
		&& (flashgpio_conf->gpio_num_info->valid[SENSOR_GPIO_FL_NOW] == 1)) {
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	CDBG("%s:%d gpio set flash_en val %d flash_now %d\n",
				...
				flashgpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_FL_EN],
				flashgpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_FL_NOW]);
	/* turn on flash led sequence flash_en 1 flash_now 1*/
	if ((flashgpio_conf->gpio_num_info->valid[SENSOR_GPIO_FL_EN] == 1)
		&& (flashgpio_conf->gpio_num_info->valid[SENSOR_GPIO_FL_NOW] == 1)) {
		gpio_set_value_cansleep(
			flashgpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_FL_EN],
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	CDBG("%s:%d gpio set flash_en val %d flash_now %d\n",
				...
				flashgpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_FL_EN],
				flashgpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_FL_NOW]);
	/* turn off flash & torchled sequence flash_en 0 flash_now 1*/
	if ((flashgpio_conf->gpio_num_info->valid[SENSOR_GPIO_FL_EN] == 1)
		&& (flashgpio_conf->gpio_num_info->valid[SENSOR_GPIO_FL_NOW] == 1)) {
		gpio_set_value_cansleep(
			flashgpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_FL_EN],
	...
	struct msm_flash_ctrl_t *flash_ctrl)
{
	...
	if (flashgpio_ctrl->cam_pinctrl_status) {
		rc = pinctrl_select_state(flashgpio_ctrl->pinctrl_info.pinctrl,
				flashgpio_ctrl->pinctrl_info.gpio_state_suspend);
		if (rc)
			pr_err("%s:%d cannot set pin to suspend state",
				__func__, __LINE__);
		devm_pinctrl_put(flashgpio_ctrl->pinctrl_info.pinctrl);
	}
	flashgpio_ctrl->cam_pinctrl_status = 0;
	msm_camera_request_gpio_table(
		flashgpio_ctrl->gpio_conf->cam_gpio_req_tbl,
		flashgpio_ctrl->gpio_conf->cam_gpio_req_tbl_size, 0);
	...
	struct msm_flash_ctrl_t *fctrl)
{
	...
	} else if ((rc == 0) && (flashtype == GPIO_FLASH)) {
		fctrl->flash_driver_type = FLASH_DRIVER_GPIO;
		CDBG("%s: fctrl->flash_driver_type = %d \n", __func__,
				fctrl->flash_driver_type);
	}
...
static int32_t msm_flash_platform_probe(struct platform_device *pdev)
{
	...
	if (flash_ctrl->flash_driver_type == FLASH_DRIVER_PMIC) {
		...
		rc = msm_gpio_light_create_classdev(pdev, flash_ctrl);
	}
