...
#define MIN_SENSOR_INFO_LENGTH   5
...
{
	char AppStr[APP_INFO_NAME_LENTH + APP_INFO_VALUE_LENTH + MIN_SENSOR_INFO_LENGTH] = {'\0'};
	char AppName[APP_INFO_NAME_LENTH] = {'\0'};
	char AppValue[APP_INFO_VALUE_LENTH] = {'\0'};
	char* PAppStr = AppStr;
	int name_lenth=0,value_lenth=0,buf_len=0,ret=-1,appinfo_len=0;
	int i = 0;
	bool flag = false;
	char* strtok = NULL;
	...
	if(0 < strlen(sensor_type[SENSOR_MAX - 1].name) )
	{
		return -1;
	}
	buf_len = strlen(buf);
	if((APP_INFO_NAME_LENTH + APP_INFO_VALUE_LENTH ) < buf_len)
	{
		return -1;
	}
	appinfo_len  = buf_len + MIN_SENSOR_INFO_LENGTH;
	memset(AppStr, 0, appinfo_len);
	snprintf((char *)AppStr, buf_len + MIN_SENSOR_INFO_LENGTH -1, "%s", buf);
	strtok = strsep(&PAppStr, APP_NAME_VALUE_SPLIT_CHAR);
	if (strtok != NULL)
	{
