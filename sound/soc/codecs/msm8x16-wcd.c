...
static const struct msm8x16_wcd_reg_mask_val
        msm8x16_sla_wcd_codec_reg_init_val[] = {

        /* Initialize current threshold to 350MA
         * number of wait and run cycles to 4096
         */
        {MSM8X16_WCD_A_ANALOG_RX_COM_OCP_CTL, 0xFF, 0x3F},
        {MSM8X16_WCD_A_ANALOG_RX_COM_OCP_COUNT, 0xFF, 0xFF},
};
...
static void msm8x16_wcd_codec_init_reg(struct snd_soc_codec *codec)
{
	...
	of_audio_node = of_find_compatible_node(NULL, NULL, "hw,hw_audio_info");
	if (!of_audio_node) {
		dev_err(of_audio_node, "%s: audio info not find!\n"
						, __func__);
	} else {
		 ret = of_property_read_string(of_audio_node, "product-identifier", &ptype);
		 if (ret) {
			 dev_err("%s: Can not find string about: \"product-identifier\"\n"
								,__func__);
		} else {
			if (!strncmp("sla", ptype, strlen("sla"))) {
				for (i = 0; i < ARRAY_SIZE(msm8x16_sla_wcd_codec_reg_init_val); i++)
					snd_soc_update_bits(codec,
							    msm8x16_sla_wcd_codec_reg_init_val[i].reg,
							    msm8x16_sla_wcd_codec_reg_init_val[i].mask,
							    msm8x16_sla_wcd_codec_reg_init_val[i].val);
			return;
			}
		}
	}
